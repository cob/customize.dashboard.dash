...
Added new fields: Details and Link
* Type:
* Details (group)
* Link

Move the duplicable field **Board** to the correct condition inside the **Details** group

```
NEW
Type : $[*Dashboard,Link] Mandatory

Details : $group $expanded $style[field-actions-hide,children-mx-0,children-bg-none,children-border-0]
Details > [Type=Link] > Link : Mandatory

ALTERED (Move board field)
Details > [Type=Dashboard] > Board
```

#### 6.93.0

Added a new `UpdateOnDrop` option to the dashboard. This allows the user to specificy (in seconds) the time after which the dash queries should be updated after an item is dropped (via drag & drop) - only when a concurrent script is called.

```
NEW
DashboardCustomize [=DragDrop] > UpdateOnDragDrop : $number(2) $help[If you wish to for the dashboard to update the queries after a drop is performed, specify the number of seconds after which the update should happen.]
```

#### 6.89.1
- The `listSort` Handlebars helper was updated to make the field parameter optional. It now supports sorting arrays of objects by a specific field or arrays of primitive values directly. Invalid or non-array inputs are safely handled by returning an empty array.

Can now be used like:
```
{{listSort myArray "name" "asc"}}   // sorts objects by 'name' ascending
{{listSort myArray "" "desc"}}      // sorts primitive values descending
```

#### 6.89.0
- New handlebar function
...
#### 6.87.0
- Add `StopOverflow` option to instance viewer. 
    - When `StopOverflow` is not toggled (default), `!overflow-auto` is applied. When it is toggled, we removed the `!overflow-auto` and replace it with the old default behaviour, `overflow-auto`.
    - This fixes the bug where refs inside instance viewer instances would clip out of the page, since overflow was set to hidden.

```
ALTERED
Board > Component > [==InstanceViewer] InstanceViewerCustomize : $[Classes,Tag,HideSidenav,StopOverflow] $multiple
```

#### 6.86.0
- ImageViewerURL field is now duplicable.
    ...
    - Meaning the component receives a list of image URLs and using an internal list index we traverse those images to know which one to show currently. E.g:
```
{{#each this.imagens}} 
...
```
ALTERED
* Board > Component > [=ImageViewer] ImageViewerURL > * Board > Component > [=ImageViewer] ImageViewerURL (*DUPLICABLE*)
```
